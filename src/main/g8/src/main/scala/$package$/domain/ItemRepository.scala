...
object ItemRepository:
  def add(
      data: ItemData
    ): ZIO[ItemRepository, RepositoryError, ItemId] =
    ZIO.serviceWithZIO[ItemRepository](_.add(data))

  def delete(
      id: ItemId
    ): ZIO[ItemRepository, RepositoryError, Long] =
    ZIO.serviceWithZIO[ItemRepository](_.delete(id))

  def getAll(
    ): ZIO[ItemRepository, RepositoryError, List[Item]] =
    ZIO.serviceWithZIO[ItemRepository](_.getAll())

  def getById(
      id: ItemId
    ): ZIO[ItemRepository, RepositoryError, Option[Item]] =
    ZIO.serviceWithZIO[ItemRepository](_.getById(id))

  def update(
      itemId: ItemId,
      data: ItemData,
    ): ZIO[ItemRepository, RepositoryError, Option[Unit]] =
